        ...
        <div className="bg-brand-purple text-white/90 h-[40px] px-6 text-[10px] md:text-[11px] font-medium flex items-center border-b border-white/10">
          <div className="max-w-7xl mx-auto w-full flex justify-between items-center uppercase tracking-wider">
            ...
            <div className="flex items-center gap-4 shrink-0">
              <a href="https://www.instagram.com/labkalibrasi.jsa" className="hover:text-brand-green transition-colors"><InstagramIcon className="w-3.5 h-3.5" /></a>
              <a href="https://www.tiktok.com/@cv_jasa_sukses_abadi" className="hover:text-brand-green transition-colors"><TiktokIcon className="w-3.5 h-3.5" /></a>
            </div>
          </div>
        </div>
              ...
              <div className="relative" onMouseEnter={() => setActiveDropdown(true)} onMouseLeave={() => setActiveDropdown(false)}>
                ...
                <div className={`absolute top-full -left-4 w-56 pt-4 transition-all ${activeDropdown ? "opacity-100 visible" : "opacity-0 invisible"}`}>
                  <div className="bg-white rounded-xl shadow-2xl border border-slate-100 overflow-hidden p-2">
                    {services.map((item) => (
                      <Link key={item} href={`/services/${toSlug(item)}`} className="block px-4 py-2 text-sm text-brand-purple hover:bg-slate-50 hover:text-brand-green rounded-lg font-bold transition-colors">
                        {item}
                      </Link>
                    ))}
                  </div>
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="flex flex-col">
              ...
              <div className="mt-4 space-y-4 border-l-4 border-brand-green pl-6 py-2">
                {services.map(s => (
                  <Link key={s} href={`/services/${toSlug(s)}`} className="block text-lg font-bold text-slate-600" onClick={() => setIsOpen(false)}>
                    {s}
                  </Link>
                ))}
              </div>
